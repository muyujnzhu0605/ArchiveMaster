...
| 异后缀同名文件清理      | 当目录中存在某后缀文件（如.dng）时，自动删除同名不同后缀的关联文件（如.jpg）                           | 当拍摄了RAW+JPG格式的照片时，若决定处理RAW文件，那么JPG格式是多余的，需要删除。 | `TwinFileCleaner`  | [照片归档工具](https://github.com/autodotua/PhotoArchivingTools) | 已完成   |
