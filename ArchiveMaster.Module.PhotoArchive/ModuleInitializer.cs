...
namespace ArchiveMaster
{
    public class ModuleInitializer : IModuleInitializer
    {
        ...
        [
            new ToolPanelInfo(typeof(TimeClassifyPanel), ModuleName, "根据时间段归档", "识别目录中相同时间段的文件，将它们移动到相同的新目录中",
                baseUrl + "archive.svg"),
            new ToolPanelInfo(typeof(UselessJpgCleanerPanel), ModuleName, "删除多余JPG", "删除目录中存在同名RAW文件的JPG文件",
                baseUrl + "jpg.svg"),
            new ToolPanelInfo(typeof(RepairModifiedTimePanel), ModuleName, "修复文件修改时间",
                "寻找EXIF信息中的拍摄时间与照片修改时间不同的文件，将修改时间更新闻EXIF时间", baseUrl + "time.svg"),
            new ToolPanelInfo(typeof(PhotoSlimmingPanel), ModuleName, "创建照片集合副本", "复制或压缩照片，用于生成更小的照片集副本",
                baseUrl + "zip.svg"),
        ];
